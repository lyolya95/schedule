...
export const TableSchedule = (props: any) => {
        ...
        const visibleData = data // формируем отоброжаемые данные для таблицы
            ...
            .filter((item) => !hiddenData.includes(item.key))
                ...
                if (rowClassName.indexOf(classSel) !== -1) {
                    ...
                    if (event.shiftKey) {
                        if (hiddenRowKeys.length !== 0) {
                            ...
                            //const parent = selRow.closest('.ant-table-tbody');
                            //const prevSelRow = document.querySelector(`[data-row-key="${lastKey}"]`);
                            //console.log('Prev selected row', prevSelRow);
                            let clazz = '';
                            visibleData.forEach((item) => {
                                //const key = item.key ? `${item.key}` : '0';
